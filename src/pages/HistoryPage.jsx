...
import { getBurnoutBadgeClass, getBurnoutLevelKey } from '../utils/prediction'
...
export default function HistoryPage() {
  ...
  const getBurnoutLabel = (level) => {
    return t(`burnout.${getBurnoutLevelKey(level)}`)
  }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {isLoading ? (
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-12 text-center border border-slate-200 dark:border-slate-700 shadow-sm">
            ...
            <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-2">{t('history.empty_title')}</h2>
            <p className="text-slate-500 dark:text-slate-400 mb-6">{t('history.empty_desc')}</p>
                  ...
                  <div>
                    <div className="flex items-center gap-3 mb-2">
                      <span className={`text-xs font-bold px-3 py-1 rounded-full border ${getBurnoutBadgeClass(pred.burnout_level)}`}>
                        {getBurnoutLabel(pred.burnout_level).toUpperCase()}
                      </span>
                      <span className="text-sm font-medium text-slate-500 dark:text-slate-400">
                        {pred.created_at ? new Date(pred.created_at).toLocaleDateString(localeCode, {
                          day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit'
                        }) : t('history.date_unavailable')}
                      </span>
                    </div>
                    <p className="text-slate-900 dark:text-white font-semibold">{t('history.stress_level')}: {pred.stress_level}/10</p>
                    {!pred.id && (
                      <p className="text-xs text-red-500 dark:text-red-400 mt-2 font-semibold">
                        {t('history.missing_id')}
                      </p>
                    )}
                  </div>
