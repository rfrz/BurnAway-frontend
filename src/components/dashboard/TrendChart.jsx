...
import { getBurnoutLevelKey, normalizeBurnoutLevel } from '../../utils/prediction'
...
const formatChartDate = (value, localeCode) => {
  const date = new Date(value)
  if (Number.isNaN(date.getTime())) return '-'
  return date.toLocaleDateString(localeCode, { weekday: 'short' })
}

const buildTrendData = (data, localeCode, t) => {
  ...
  return [...data].slice(0, 7).reverse().map((item) => {
    ...
    return {
      name: formatChartDate(item.created_at, localeCode),
      stres: Number(item.stress_level) || 0,
      burnoutScore: BURNOUT_SCORES[burnoutLevel] ?? null,
      burnoutLevel: t(`burnout.${getBurnoutLevelKey(burnoutLevel)}`)
    }
  })
}
...
export default function TrendChart({ data }) {
  ...
  const chartData = buildTrendData(data, localeCode, t)
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm border border-slate-200 dark:border-slate-700 h-full flex flex-col transition-colors duration-300">
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h3 className="text-lg font-bold text-slate-900 dark:text-white">{t('dashboard.trend_title')}</h3>
